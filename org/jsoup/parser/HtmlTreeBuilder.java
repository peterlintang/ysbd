  ...
  extends TreeBuilder
{
  ...
  Element insert(Token.StartTag paramStartTag)
  {
    if ((paramStartTag.isSelfClosing()) && (!Tag.isKnownTag(paramStartTag.name())))
    {
      ...
      process(new Token.EndTag(paramStartTag.tagName()));
  ...
  {
    Iterator localIterator = this.stack.descendingIterator();
    while ((localIterator.hasNext()) && (!((Element)localIterator.next()).nodeName().equals(paramString))) {
      localIterator.remove();
    }
  }
...
/* Location:              E:\project\angent\dex2jar-2.0\classes-dex2jar.jar!\org\jsoup\parser\HtmlTreeBuilder.class
