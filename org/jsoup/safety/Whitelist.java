...
{
  private Map<Whitelist.TagName, Set<Whitelist.AttributeKey>> attributes = new HashMap();
  private Map<Whitelist.TagName, Map<Whitelist.AttributeKey, Whitelist.AttributeValue>> enforcedAttributes = new HashMap();
  private boolean preserveRelativeLinks = false;
  private Map<Whitelist.TagName, Map<Whitelist.AttributeKey, Set<Whitelist.Protocol>>> protocols = new HashMap();
  private Set<Whitelist.TagName> tagNames = new HashSet();
  ...
  private boolean testValidProtocol(Element paramElement, Attribute paramAttribute, Set<Whitelist.Protocol> paramSet)
  {
  ...
  boolean isSafeAttribute(String paramString, Element paramElement, Attribute paramAttribute)
  {
    ...
    if ((this.attributes.containsKey(localTagName)) && (((Set)this.attributes.get(localTagName)).contains(localAttributeKey)))
    {
      if (this.protocols.containsKey(localTagName))
      {
        ...
        return (!paramString.containsKey(localAttributeKey)) || (testValidProtocol(paramElement, paramAttribute, (Set)paramString.get(localAttributeKey)));
      }
      return true;
    }
    return (!paramString.equals(":all")) && (isSafeAttribute(":all", paramElement, paramAttribute));
  }
...
/* Location:              E:\project\angent\dex2jar-2.0\classes-dex2jar.jar!\org\jsoup\safety\Whitelist.class
